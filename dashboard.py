...
with col1:
    ...
    fig = px.bar(category_sales, x='Category', y='Total Sales', color='Category', color_discrete_sequence=px.colors.qualitative.Set2)
    st.plotly_chart(fig, use_container_width=True)

with col2:
    st.subheader("Sales by Segment")
    segment_sales = df_filtered.groupby('Segment')['Total Sales'].sum().reset_index()
    fig = px.pie(segment_sales, names='Segment', values='Total Sales', color_discrete_sequence=px.colors.qualitative.Pastel)
    st.plotly_chart(fig, use_container_width=True)
...
with col1:
    ...
    fig = px.line(monthly_sales, x='Month', y='Total Sales', markers=True, color_discrete_sequence=['steelblue'])
...
with col2:
    ...
    fig = px.bar(ship_sales, x='Total Sales', y='Ship Mode', orientation='h', color='Ship Mode', color_discrete_sequence=px.colors.qualitative.Set3)
    st.plotly_chart(fig, use_container_width=True)
...
with col1:
    ...
    fig = px.bar(state_sales, x='Total Sales', y='State', orientation='h', color_discrete_sequence=['coral'])
    st.plotly_chart(fig, use_container_width=True)

with col2:
    st.subheader("Profit by Category")
    profit_cat = df_filtered.groupby('Category')['Unit Profit'].sum().reset_index()
    fig = px.pie(profit_cat, names='Category', values='Unit Profit', hole=0.4, color_discrete_sequence=px.colors.qualitative.Set1)
    st.plotly_chart(fig, use_container_width=True)
